...
┌─────────────────────┐             ┌──────────────────────┐
│   req_ack_sender    │             │   req_ack_receiver   │
│                     │──req──────→│  [2FF sync for req]  │
│  data[7:0] valid    │──data[7:0]→│                      │
│                     │←──ack──────│  [2FF sync for ack]  │
│                     │             │                      │
└─────────────────────┘             └──────────────────────┘
        ↑                                     ↑
   CDC crossing 1:                      CDC crossing 2:
   req (toggle) → BCLK               ack (toggle) → ACLK
   data held stable                  data captured safely
